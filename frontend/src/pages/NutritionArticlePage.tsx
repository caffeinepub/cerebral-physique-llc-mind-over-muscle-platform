...
import { ArrowLeft, Lock, Calendar, User, Leaf } from 'lucide-react';
import { useGetNutritionArticle, useGetMyMembership, useIsCallerAdmin } from '../hooks/useQueries';
...
function MediaGallery({ imageUrls, videoUrls }: { imageUrls: string[]; videoUrls: string[] }) {
  ...
  return (
    <div className="space-y-6 mt-6">
      {imageUrls.length > 0 && (
        <div className={`grid gap-4 ${imageUrls.length === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'}`}>
          {imageUrls.map((url, i) => (
            <img
              key={i}
              src={url}
          ...
          <header className="mb-8">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {new Date(Number(article.createdAt) / 1_000_000).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                })}
          ...
          {isLocked ? (
            <div className="space-y-6">
              ...
              <div className="bg-card border border-border rounded-2xl p-8 text-center space-y-4">
                ...
                <h3 className="text-xl font-bold text-foreground">Members Only Content</h3>
                <p className="text-muted-foreground max-w-md mx-auto">
                  This nutrition article is exclusively available to Cerebral Physique members.
                  Join today to unlock all premium content.
                </p>
                {!isAuthenticated ? (
            ...
            <div className="space-y-6">
              ...
              {/* Media */}
              {(article.media?.imageUrls?.length > 0 || article.media?.videoUrls?.length > 0) && (
                <MediaGallery
