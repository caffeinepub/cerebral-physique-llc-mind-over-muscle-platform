...
import type { BlogMedia, NutritionArticle } from "../../backend";
import {
                  ...
                  <div className="flex gap-3 mt-1.5 text-xs text-muted-foreground">
                    {article.media?.imageUrls?.length > 0 && (
                      <span className="flex items-center gap-1">
                        ...
                        {article.media.imageUrls.length} image
                        {article.media.imageUrls.length !== 1 ? "s" : ""}
                      </span>
                    )}
                    {article.media?.videoUrls?.length > 0 && (
                      <span className="flex items-center gap-1">
                        <Video className="w-3 h-3" />
                        {article.media.videoUrls.length} video
                        {article.media.videoUrls.length !== 1 ? "s" : ""}
                      </span>
                    )}
                  </div>
                </div>
